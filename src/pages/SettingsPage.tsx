        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-dark-600 border border-dark-300/50 rounded-2xl p-6 shadow-card space-y-6">
          ...
          <div className="pt-4 border-t border-dark-400">
            ...
            <div className="space-y-4">
               <div 
                 onClick={() => useUIStore.getState().toggleModule('cartoes')}
                 className="flex items-center justify-between p-4 bg-dark-500/50 rounded-xl border border-dark-300/30 cursor-pointer hover:bg-dark-500 transition-colors"
               >
                 <div>
                   <p className="text-white font-medium">Módulo de Cartões de Crédito</p>
                   <p className="text-xs text-gray-400">Controle de faturas, limites e gastos pessoais.</p>
                 </div>
                 <div className={`w-10 h-6 rounded-full relative transition-colors ${useUIStore(state => state.enabledModules.cartoes) ? 'bg-primary-600' : 'bg-gray-600'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${useUIStore(state => state.enabledModules.cartoes) ? 'left-5' : 'left-1'}`} />
                 </div>
               </div>

               <div 
                 onClick={() => useUIStore.getState().toggleModule('dividas')}
                 className="flex items-center justify-between p-4 bg-dark-500/50 rounded-xl border border-dark-300/30 cursor-pointer hover:bg-dark-500 transition-colors"
               >
                 <div>
                   <p className="text-white font-medium">Módulo de Gestão de Dívidas</p>
                   <p className="text-xs text-gray-400">Controle de clientes, cobranças e juros de empréstimos.</p>
                 </div>
                 <div className={`w-10 h-6 rounded-full relative transition-colors ${useUIStore(state => state.enabledModules.dividas) ? 'bg-primary-600' : 'bg-gray-600'}`}>
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${useUIStore(state => state.enabledModules.dividas) ? 'left-5' : 'left-1'}`} />
                 </div>
               </div>
            </div>
            <p className="text-xs text-gray-500 mt-4 italic">
              * Nota: Desativar um módulo apenas o esconde da interface para simplificar o uso. Seus dados continuam salvos.
            </p>
          </div>
